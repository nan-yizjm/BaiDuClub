...
- python语法部分，不知道或者忘记的部分有：

  - set （参考`Python_Basic.ipynb`这个文件）
  - 列表生成式（参考`Python_Basic.ipynb`这个文件）

  - 高阶函数（参考`Python_Basic.ipynb`这个文件）
...
- 除了整数和字符串在函数内不能改，列表，字典这些可以在函数内部改

- `lst.insert(position,object)`：在`list`的指定位置插入一个`object`

- `del lst[position]`可以删除`list`中下标为`position`的元素
